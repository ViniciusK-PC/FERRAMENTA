const blob = "=";

const reversed = blob.split("").reverse().join("");
const decoded = Buffer.from(reversed.replace(/^=/, ''), 'base64');

const keys = [
    '939ff21415174dc5118eb885cf5a535c',
    '2569dc84ad21c232d1ad8c6c74cdaeb3'
];

keys.forEach(key => {
    console.log(`Trying XOR with key: ${key}`);
    const keyBuf = Buffer.from(key, 'utf8');
    const xored = Buffer.alloc(decoded.length);
    for (let j = 0; j < decoded.length; j++) {
        xored[j] = decoded[j] ^ keyBuf[j % keyBuf.length];
    }
    const str = xored.toString('utf8');
    console.log("Snippet:", str.slice(0, 50).replace(/[^\x20-\x7E]/g, '.'));
    if (str.includes('{"')) {
        console.log("MATCH FOUND!");
        console.log(str);
    }
});
